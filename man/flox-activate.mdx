...
not have to run `flox activate` by hand.
...
1.  The Flox prompt hook must be installed in your shell. The hook is
    ...
    with Flox and stays dormant until the feature flag below is set. Set
    `disable_hook = true` to opt out of the hook entirely.
...
`bash`, `zsh`, `fish`, and `tcsh` are supported.
...
started unless `services.auto-start = true` is set in the manifest.
...
directory. To stop being asked for a directory, run
