...
absolute path of the directory that contains the `.flox` directory. To
cover many directories with one entry, set a glob pattern with
`flox config --set` instead (see `auto_activate_environments` in
