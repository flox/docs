...
catalog with `flox publish`. These references are resolved and locked
automatically during `flox build`. There is no separate declaration file
and no separate lock step.
