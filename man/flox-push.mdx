...
Push an environment to FloxHub to share and centrally manage it with
flox, or sync changes made locally to a remote environment.
...
syncing with `flox push`. See [`flox-edit(1)`](/man/flox-edit),
[`flox-install(1)`](/man/flox-install), and
[`flox-uninstall(1)`](/man/flox-uninstall) for editing the environment.
...
Cannot be used with `--reference`. Defaults to the current directory
...
Use ‘flox pull –copy’ to copy an existing environment and push it to a
new owner.
...
commands with the `--reference` flag. Referring to environments, that
