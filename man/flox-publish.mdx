...
Nix expression builds. Other environments reference the published
packages as `catalogs.<catalog>.<package>` in `.nix` expressions under
`.flox/pkgs/`. These references are pinned by a committed
`.flox/catalog.lock` when the project has one (see
`flox build update-catalogs`), and are otherwise resolved fresh during
the build. `flox publish` submits the entries relevant to the published
package along with the build.
