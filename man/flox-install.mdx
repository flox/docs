...
[`flox-edit(1)`](/man/flox-edit), which allows specifying a variety of
options for package installation. See
[`manifest-toml(1)`](/man/manifest.toml) for more details on the
available options.
...
`@<version>` to specify a version requirement, or `^<outputs>` to select
which outputs to install. Use `^..` to install all outputs, or a
comma-separated list such as `^bin,man` to install specific outputs. The
version constraint (`@`) and output specifier (`^`) are mutually
exclusive.

Alternatively, an arbitrary Nix flake installable, or store path may be
specified. See [`manifest-toml(1)`](/man/manifest.toml) for more
details.
