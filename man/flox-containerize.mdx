...
`labels`  
This field contains arbitrary metadata for the container, as a map of
string keys to string values following the OCI [annotation
rules](https://github.com/opencontainers/image-spec/blob/main/annotations.md#rules).

`stop-signal`  
The signal to send to the main process to stop the container (e.g.,
...
[`flox-activate`](/man/flox-activate), [`docker-load`](https://docs.docker.com/reference/cli/docker/image/load/)
