...
Run a command from a Flox Catalog package without creating an
environment.

`flox run` is designed for one-off invocations. It resolves the
...
The package name is a plain Flox Catalog attribute path (e.g. `curl`,
...
Without `--`, flags that look like options could be claimed by the wrong
side of the boundary — for example,
`flox run -p curl curl -sL http://example.com` passes `-sL` to `curl`,
but `flox run curl -p curl -- curl ...` would fail because `-p` is
...
`$FLOX_CACHE_DIR/run-gc-roots/`. Repeated invocations of the same
...
Required. The Flox Catalog package that provides the command. Accepts
plain package names only (e.g. `curl`, `ripgrep`). Version constraints
(`@`), output selectors (`^`), and custom catalogs (`/`) are not
supported in this release.

`-- <command> [<arguments>]`  
The command to run and any arguments to pass to it. Use `--` to separate
...
This release (phase 1) requires the `-p`/`--package` flag. The following
features are not yet supported and will be available in a future
release:
