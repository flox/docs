...
The Flox Catalog package that provides the command. Optional: when
omitted, the package is resolved from the command name (see “Specifying
the Package”). Accepts plain package names (e.g. `curl`, `ripgrep`) and
...
The following features are not yet supported and will be available in a
future release:
