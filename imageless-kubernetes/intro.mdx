...
Once you create a pod specification and deploy it, the pod will start up with the currently live [generation](/concepts/generations) of the environment.
...
To quickly try out Imageless Kubernetes locally, see the [examples](/imageless-kubernetes/examples/kind-demo), which uses `kind` to create a local Kubernetes cluster.
...
See the [installation](/imageless-kubernetes/install/eks) for more details on installing Imageless Kubernetes in your cluster.
