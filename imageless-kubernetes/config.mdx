...
Flox environments are accessed centrally via [FloxHub](/concepts/floxhub) and managed using the Flox CLI.

In the [introduction](/imageless-kubernetes/intro) we discussed how annotations are used to tell Imageless Kubernetes which Flox environment to run.
...
To do so, you need to first login to FloxHub using the Flox CLI using [`flox auth login`](/man/flox-auth), if you haven't done so already.
...
See the [`options.activate.mode`](/man/manifest.toml#options) option in the manifest for more details on modes.
...
A specific [generation](/concepts/generations) of an environment on FloxHub can be specified as part of the `flox.dev/environment` annotation.
