...
<Note>
  **Note**
  ...
  These options can also be passed as part of the job definition in `.gitlab-ci.yml`, see the [GitLab documentation](https://docs.gitlab.com/runner/executors/kubernetes/) for more details.
</Note>
...
See the [configuration](/imageless-kubernetes/config) page for more details on annotations.
...
where the value of the annotation is the name of an environment from [FloxHub](/concepts/floxhub).
