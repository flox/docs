...
The installer used in the [installation instructions](/imageless-kubernetes/install/self-managed) makes a backup of the original configuration
in `/etc/containerd/config.toml.bak.xx` where `xx` is an arbitrary number.

Restore the backup with:

```bash
sudo mv /etc/containerd/config.toml.bak.xx /etc/containerd/config.toml &&
...
```bash
sudo rm /usr/local/bin/containerd-shim-flox-v2
```

And finally, uninstall Flox from each node by following the instructions from the [Uninstall Flox](/install-flox/uninstall) page.
