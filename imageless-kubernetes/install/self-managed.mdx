...
Flox packages and installation instructions for `rpm` and `deb` based distributions are available from the [Install Flox](/install-flox/install) page.
...
Details about the installer can be found on its [FloxHub page](https://hub.flox.dev/flox/containerd-shim-flox-installer); the script is executed by the activation hook for the environment.
...
2. Create a symlink from the Flox environment to `/usr/local/bin`.
    ...
    ```sh
    ln -s $PWD/.flox/run/*-linux.containerd-shim-flox.run/bin/containerd-shim-flox-v2 /usr/local/bin/containerd-shim-flox-v2

    ```

    On aarch64 nodes, substitute `aarch64-linux` in the path.
