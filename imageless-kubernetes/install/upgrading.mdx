...
If a separate node group was created per the [install guidance](/imageless-kubernetes/install/eks), then upgrading Flox and the runtime shim
...
For Flox, the [Install Flox](/install-flox/install) page has details for each installation type on how to upgrade.
