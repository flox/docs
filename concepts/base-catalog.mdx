...
It is generated from a fork of [nixpkgs](https://github.com/NixOS/nixpkgs),
...
from its [nixpkgs](https://github.com/NixOS/nixpkgs) fork
and save the metadata from the evaluation.
This makes it available to users of Flox.

An additional process continuously checks newly added packages
and records actual build information (narinfo) from `cache.nixos.org`.
...
in the [environment manifest](/concepts/environments#manifesttoml) to override the defaults.
...
you can use this site: [Nixpkgs PR Tracker](https://nixpkgs-tracker.ocfox.me).

### Flox branches

Flox maintains a fork of [nixpkgs](https://github.com/flox/nixpkgs)
...
of our [fork](https://github.com/flox/nixpkgs)
(which, again, is not the same thing as `nixos-unstable`).

- `unstable` is reset to upstream `unstable` daily
- `staging` is reset to the `unstable` branch of our [fork](https://github.com/flox/nixpkgs) each Saturday
- `stable` is reset to the `staging` branch of our [fork](https://github.com/flox/nixpkgs) on the first Saturday of the month
- `lts` is reset to the `stable` branch of our [fork](https://github.com/flox/nixpkgs) on the first Saturday in January and July (every 6 months)
...
a tag is created in the form of `<stability>.<date>`.
...
that contains a specific version,
...
in effect keeping every version in the past 3 years available.
