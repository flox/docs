...
Both files should be committed to version control. The TOML file is authored by hand; the lockfile is generated and updated by the `flox build update-catalogs` command.

## Source-ref (Git) catalogs

A Git catalog points at a remote repository that contains Flox package definitions (a `.flox/pkgs/` directory). Declare one in your `nix-builds.toml` under the `[catalogs]` section:
...
Once catalogs are declared in `nix-builds.toml` and the lockfile is generated, you access imported packages through the `catalogs` argument in your Nix expressions.
...
This resolves each catalog declaration in `nix-builds.toml` and writes the pinned revisions to `nix-builds.lock`.
...
After updating, commit both `.flox/nix-builds.toml` and `.flox/nix-builds.lock` to version control.

<Note>
  If you run `flox build` without first generating or updating the lockfile, the build will fail with an error indicating that catalogs have not been resolved.
</Note>
