...
Catalog imports allow your [Nix expression builds](/concepts/nix-expression-builds) to depend on packages provided by external sources: FloxHub users and organizations that publish reusable packages, or remote repositories containing Nix expression definitions.
...
Both files must be tracked by Git (`git add`ed) and should be committed to version control. The TOML file is authored by hand; the lockfile is generated and updated by the [`flox build update-catalogs`](/man/flox-build-update-catalogs) command.

## Catalog types

### FloxHub catalogs

A FloxHub catalog references packages published to FloxHub via [`flox publish`](/concepts/publishing). The catalog name corresponds to the FloxHub user or organization that published them.
...
A source-ref catalog points at any [Nix source reference](https://nix.dev/manual/nix/latest/language/builtins.html#source-types): a Git repository, tarball, or other supported source type. The most common case is a Git repository that contains Flox package definitions (a `.flox/pkgs/` directory).
...
As a shorthand, you can provide a single `url` field using [Nix source reference syntax](https://nix.dev/manual/nix/latest/language/builtins.html#source-types) (the `git+` prefix encodes the type):
...
The supported source types and their fields are documented in the [Nix manual](https://nix.dev/manual/nix/latest/language/builtins.html#source-types).
...
Other source types (`github`, `gitlab`, `tarball`, etc.) accept their own fields; for example, `github` and `gitlab` support a `host` field for self-hosted instances. See the [Nix source types reference](https://nix.dev/manual/nix/latest/language/builtins.html#source-types) for the full list.
...
This resolves each catalog declaration in `.flox/nix-builds.toml` and writes the pinned revisions to `.flox/nix-builds.lock`. See [`flox-build-update-catalogs(1)`](/man/flox-build-update-catalogs) for details.
