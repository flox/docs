...
A manifest can carry a `description`, a short summary of what the environment provides and how to use it.
The description travels with the environment when you push, pull, or include it, so the context stays with the manifest rather than in a separate README.
It has no effect on activation.
See [`description`](/man/manifest.toml#description).

```toml title=".flox/env/manifest.toml"
schema-version = "1.17.0"
description = "Node.js toolchain for the example project"
