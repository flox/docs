...
[Environments](/concepts/environments) are a central concept in Flox,
...
| --- | --- | :---: | :---: |
| Subshell | `flox activate` | ✅ | ✅ |
| In-place | `eval "$(flox activate)"` | ✅ | ✅ |
| Shell command | `flox activate -c` | ✅ | ✅ |
| Exec | `flox activate --` | ✅ | ❌ |
...
is resumed and you are put back into it.
...
sequenceDiagram
    participant U as User shell
    U->>U: source <(flox activate)
    Note over U: Back to you
```
...
<Tip>
  An in-place activation like this also installs the prompt hook used by
  [auto-activation](/concepts/auto-activation), which can activate an
  environment automatically when you enter its directory.
</Tip>
...
down to the subshell (unless you use [special options like `export -f`](https://www.gnu.org/software/bash/manual/html_node/Bourne-Shell-Builtins.html#index-export)).
...
sequenceDiagram
    ...
    U->>P: call "flox activate"
    rect rgb(235, 240, 250)
      Note over P: flox activate
      P->>P: exec bash activation_script.sh
    end
    ...
    rect rgb(240, 240, 245)
      Note over P: FLOX_SHELL
      P->>P: source [profile] scripts
      rect rgb(228, 232, 240)
        Note over P: Do work
        P->>P: Do your work inside the shell
      end
    end
    P->>U: exit
```
...
you only need to worry about the syntax and oddities of one shell when writing
this script.
...
[[profile] section of your manifest](/man/manifest.toml#profile).
...
- Can define environment variables that require shell-specific logic.

In short, define environment variables in `[vars]` when you can express their
values directly or by referencing other entries. Use `hook.on-activate` for
environment-wide setup or values that require shell logic. Use `[profile]` for
shell-specific needs, aliases, or third-party scripts that your shell must source.
...
sequenceDiagram
    ...
    participant F as Somewhere
    participant S2 as Second shell
    S1->>S1: flox activate
    S1->>F: save environment variables
    Note over S1: Activated
    S2->>S2: flox activate
    F->>S2: restore environment variables
    Note over S2: Activated
```
