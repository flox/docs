...
Once you've built a package with the [`flox build`](/man/flox-build) command, you likely want to _use_ it somewhere.
The [`flox publish`](/man/flox-publish) command gives you the ability to upload packages to your private catalog so that you can _install_ them into your environments.
...
- The Flox environment performing the build must be tracked in a git repository.
...
As a reminder, the Base Catalog is the built-in [Catalog](/concepts/packages-and-catalog) provided by Flox.
...
The package metadata is uploaded to Flox servers so that the Flox CLI can see that it's available via the [`flox search`](/man/flox-search), [`flox show`](/man/flox-show), and [`flox install`](/man/flox-install) commands.
