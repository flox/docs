...
<Note>
  **Note**
  ...
  The Flox [base Catalog](/concepts/base-catalog) evaluates a subset of packages from that fork
...
The Flox [environment manifest](/concepts/environments#environment-uses) above defines the `openssl` and `curl` packages. Flox resolves both against the same `nixpkgs` commit, selecting v3.6.1 of `openssl` and v8.18.0 of `curl`.
...
flowchart LR
  ...
  subgraph ml ["Group: ml"]
    direction TB
    mp["python311, numpy, scipy"]
    mr["Catalog rev c5296fd…<br/>(nixpkgs commit 2025-07-22)"]
  end
```
  ...
  "packages": [
    {
      ...
      "attr_path": "git",
      "version": "2.47.1",
      "group": "toplevel", // Group assignment — all packages in the same group share the same `rev`
      "rev": "b40629efe5d6ec48dd1efba650c797ddbd39ace0", // The catalog revision (nixpkgs commit) this group resolved to
    ...
    },
    {
      ...
      "attr_path": "gum",
      "version": "0.17.0",
      "group": "tools", // Different group — may resolve to a different catalog revision
      "rev": "a1b2c3d4e5f6...", // May differ from the `toplevel` group's revision
...
When you use `flox build` with [manifest builds](/concepts/manifest-builds), only packages in the `toplevel` group are available as build dependencies. Packages in named groups are **not** accessible in build commands:
