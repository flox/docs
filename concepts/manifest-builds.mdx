...
See the [builds concept](/concepts/builds) page for an overview of the different types of builds and how to perform them.
...
You can optionally define a `version` and `description` for the package to provide metadata used during the [publish](/concepts/publishing) process.
See the [build section of the manifest reference](/man/manifest.toml#build) for more details on the available options.
...
```bash
mkdir -p $out/bin && cp target/release/myproject $out/bin/myproject
```

The contents of the `$out` directory should adhere to the [Filesystem Hierarchy Standard (FHS)](https://en.wikipedia.org/wiki/Filesystem_Hierarchy_Standard), which is just the official name for the familiar `bin`, `lib`, etc directories you may be familiar with from using Unix-based systems.
...
Here's an abbreviated example demonstrating how to achieve this pattern with Go (see the [Go cookbook page](/languages/go#build-with-flox) for more precise instructions):
...
Once that build is done, the package can be [published](/concepts/publishing) so that your or anyone else in your [organization](/concepts/organizations) can install it into their environment.
...
See the [organizations](/concepts/organizations) page for more details on organizations.
...
Once this package is published, any environment that installs it would then be able to reference the config file as `$FLOX_ENV/etc/nginx.conf`.

### Example: protocol buffers

Say that your organization uses [grpc](https://grpc.io/) to communicate between services.
...
For example, [this section](/languages/go#build-with-flox) contains an example build for the Go language.
