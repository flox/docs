...
These names cannot conflict with [manifest builds](/concepts/manifest-builds) in the same environment and will result in an error if they do.
...
Nix expression builds can depend on packages published to FloxHub catalogs. Rather than vendoring dependencies or using Nix flake inputs directly, you reference a catalog package via the `catalogs` argument in your expressions. Flox resolves and locks these references automatically during `flox build` and `flox publish`.

See [Catalog imports](/concepts/catalog-imports) for full documentation and examples.
