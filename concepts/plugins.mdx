...
nothing happens — Flox doesn't cross-reference the two. Install a plugin
but skip its configuration, and activation fails: the plugin's script
calls `flox_plugin_data`, finds no table for its name, and exits non-zero.
This is by design — a plugin that depends on its configuration should fail
loudly rather than silently run with defaults it was never given.
...
while IFS= read -r name; do
  path="$(jq -r --arg n "$name" '.[$n]' <<< "$_data")"
  export "$name=$(vault kv get -field=value "secret/$path")"
done < <(jq -r 'keys[]' <<< "$_data")
```
...
missing — which aborts activation, since `profile.d` scripts run under
`set -e`. Parse the JSON however you like; `jq` is the obvious choice.
