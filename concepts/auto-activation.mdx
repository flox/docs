...
directory that contains a `.flox` subdirectory, and deactivates it when you leave.
...
<Note>
  ...
  `eval "$(flox activate -r owner/default)"`), the hook is already installed
  and auto-activation is already active.
  Run `flox config --set disable_hook true` to opt out of the hook entirely,
  which disables auto-activation along with other features like
  `flox deactivate`.
</Note>

## How it works

Once the hook is installed, it runs on every prompt (or directory change,
...
| **Gate** | None — you chose to activate | Requires an allowed environment |
