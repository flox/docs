...
Organizations can connect their own identity provider, so members sign in with their work accounts and join the organization automatically on first sign-in. Owners set the connection up themselves from the organization's settings page. See [Enterprise SSO](/concepts/enterprise-sso) for how sign-in works and how to set it up.
