...
To fix this, add `floxhub-1` to your Nix configuration's trusted public keys, the same way you would [trust any other public key](#trust-a-public-key-to-install-published-artifacts) — see the [generic Nix install instructions](/install-flox/install#generic-nix) for the exact key value and the config syntax for your setup.
...
<Warning>
  **The path to the private key must be configured with an absolute path for security purposes.**
...
<Tip>
  The config key uses snake case (`signing_private_key`) while the CLI flag uses kebab case (`--signing-key`).
</Tip>
