...
By default, Flox provides a pre-configured Catalog Store to each organization as part of the organization's private Catalog.

A user-provided Catalog Store is an AWS S3 bucket
or any S3 compatible service, like [MinIO](https://min.io/product/s3-compatibility)
or [Backblaze B2](https://www.backblaze.com/cloud-storage). (For the
...
- [Amazon Simple Storage Service API Reference](https://docs.aws.amazon.com/AmazonS3/latest/API/Welcome.html)
...
Use the `aws configure` or `aws configure sso` command [as described in the CLI reference](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/configure/index.html#configure) to set those same values, and ensure that the AWS profile and region match those configured for the S3 bucket.
...
See the [signing keys](/customer/signing-keys) Cookbook page for instructions on configuring your signing keys.
