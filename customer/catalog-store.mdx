...
or any S3 compatible service, like [MinIO][minio-s3-compatible]
or [Backblaze B2][backblaze-b2-cloud-storage]. (For the
...
- [What is Amazon S3?][amazon-s3]
- [AWS S3 CLI Reference][aws-cli-reference-s3]
- [Amazon Simple Storage Service API Reference][aws-s3-api-reference]
...
Use the `aws configure` or `aws configure sso` command [as described in the CLI reference][aws-cli-configure-command] to set those same values, and ensure that the AWS profile and region match those configured for the S3 bucket.
