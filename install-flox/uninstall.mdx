...
<Tabs>
  ...
  The install script installs Flox with your system package manager, or
  through Nix if Nix was already present. Uninstall with the method that
  matches how the script installed Flox:
  ...
  * If Flox was installed through an existing Nix, remove it from your Nix
    profile:
    ...
    If that reports no matching entry, run `nix profile list` to find the
    right name (on older Nix, remove by index or with
    `nix profile remove github:flox/flox/latest`).
    To also remove the cache configuration the script added,
    delete the lines under `# Added by the Flox installer (get.flox.dev)`
