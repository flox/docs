...
export function useFloxVersion() {
  ...
  useEffect(() => {
    const KEY = 'flox_latest_version';
    const TTL = 6 * 60 * 60 * 1000;
    try {
      const item = localStorage.getItem(KEY);
      if (item) {
        const { value, ts } = JSON.parse(item);
        if (Date.now() - ts < TTL) { setVersion(value); return; }
      }
    } catch {}
    fetch('https://downloads.flox.dev/by-env/stable/LATEST_VERSION')
      .then(r => r.text())
      .then(text => {
        const v = text.trim();
        try { localStorage.setItem(KEY, JSON.stringify({ value: v, ts: Date.now() })); } catch {}
        setVersion(v);
      })
      .catch(() => {
        try {
...
export function FloxVersionedUrl({ base, children }) {
  ...
  return <a href={base.replace('{VERSION}', version)}>{children}</a>;
}

export function FloxVersionOutput() {
  const version = useFloxVersion();
  return (
    <div style={{fontFamily:'ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace',background:'#f6f8fa',color:'#1f2328',borderRadius:'6px',padding:'16px',fontSize:'14px',lineHeight:'1.45',overflowX:'auto',marginBottom:'16px',whiteSpace:'pre'}}>
      {'$ flox --version\n'}{version ?? ''}
    </div>
  );
}

export function MacPkgCards() {
  const v = useFloxVersion();
  return (
    <CardGroup cols={2}>
      <Card
        title="Apple Silicon"
        icon="apple"
        href={v ? `https://downloads.flox.dev/by-env/stable/osx/flox-${v}.aarch64-darwin.pkg` : undefined}
      >
        aarch64-darwin
      </Card>
      <Card
        ...
        icon="microchip"
        href={v ? `https://downloads.flox.dev/by-env/stable/osx/flox-${v}.x86_64-darwin.pkg` : undefined}
      >
        x86_64-darwin
      </Card>
    </CardGroup>
...
<Note>
  **Verify your download**
  ...
  You can find the{' '}
  <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/flox-{VERSION}.SHA256SUMS">SHA256 checksums</FloxVersionedUrl>{' '}
  and{' '}
  <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/flox-{VERSION}.SHA512SUMS">SHA512 checksums</FloxVersionedUrl>{' '}
  for Flox <FloxVersion /> online.
</Note>
<Tabs>
  <Tab title="MacOS (Pkg)">
  ...
  **Download and install the package**
  ...
  1. Download flox.deb for your system architecture:
       ...
       <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/deb/flox-{VERSION}.x86_64-linux.deb">64-bit Intel/AMD</FloxVersionedUrl>
       <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/deb/flox-{VERSION}.aarch64-linux.deb">64-bit ARM</FloxVersionedUrl>
  ...
  **Download and install the package**
  ...
  1. Download flox.rpm for your system architecture:
       ...
       <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/rpm/flox-{VERSION}.x86_64-linux.rpm">64-bit Intel/AMD</FloxVersionedUrl>
       <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/rpm/flox-{VERSION}.aarch64-linux.rpm">64-bit ARM</FloxVersionedUrl>
