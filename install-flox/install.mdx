...
export function FloxVersionOutput() {
  const version = useFloxVersion();
  return (
    <pre style={{borderRadius:'0.375rem',padding:'1rem',fontSize:'0.875rem',lineHeight:'1.5',overflowX:'auto',marginBottom:'1rem'}}>
      <code>{'$ flox --version\n'}{version}</code>
    </pre>
  );
}
  ...
  1. Download installer for Macs with
      ...
      <CardGroup cols={2}>
        <Card title="Apple Silicon" icon="apple">
          <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/osx/flox-{VERSION}.aarch64-darwin.pkg">aarch64-darwin</FloxVersionedUrl>
        </Card>
        <Card title="Intel processors" icon="microchip">
          <FloxVersionedUrl base="https://downloads.flox.dev/by-env/stable/osx/flox-{VERSION}.x86_64-darwin.pkg">x86_64-darwin</FloxVersionedUrl>
        </Card>
      </CardGroup>
