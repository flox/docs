...
<Tabs>
  ...
  through Nix if Nix was already present. Follow the tab that matches how
  the script installed Flox: [MacOS (Pkg)](#macos-pkg) on MacOS,
  [Debian](#debian) on Debian- and Ubuntu-family systems, or
  [RPM](#rpm) on Fedora- and RHEL-family systems. If Flox was installed
  through an existing Nix, remove it with:
...
<Tabs>
  <Tab title="Determinate Nix Installer">
  ...
  <Tab title="Other Nix installations">
...
3. Re-install Flox following the instructions on the [Install](/install-flox/install) page.
...
[Install](/install-flox/install) page.
