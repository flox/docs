...
2. If your `npm build` already produces a binary that can be executed directly, you can also copy or link that to `$out/bin`. Note that only binaries in `$out/bin` are wrapped to ensure they run within a consistent environment.
...
[build-concept]: /concepts/builds
[pure-builds-section]: /concepts/manifest-builds#pure-builds
