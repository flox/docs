...
The full example environment can be found in the [floxenvs repository](https://github.com/flox/floxenvs/tree/main/rust).
...
description = "Rust toolchain with rust-analyzer, clippy, and rustfmt"
...
The typical Rust developer probably uses [rustup](https://rustup.rs) to manage their
toolchains.
...
An example flake is provided at [zmitchell/rust-toolchains](https://github.com/zmitchell/rust-toolchains).
...
- `esp32-riscv-no-std`, which provides a nightly toolchain with support for the [ESP32-C3](https://www.espressif.com/en/products/socs/esp32), a microprocessor based on the [RISC-V](https://en.wikipedia.org/wiki/RISC-V) architecture.
...
```toml title="manifest.toml"
[build.myproject]
# ... other build fields above ...
runtime-packages = ["gcc"]   # comma-delimited array; add any other packages your binary needs at runtime
```
