...
Activating a Flox [environment](/concepts/environments) places you into a subshell.
...
For more details on what it looks like to develop in Rust with Flox, see the [Rust language guide](/languages/rust).

If you'd like to follow along with a real Flox environment, create an environment via [`flox init`](/man/flox-init) and install the tools as shown below:
...
For a full discussion of what logic to place in which section and why, see the [activation concept page](/concepts/activation).
Otherwise, try this:


- Are you setting an environment variable?
    - Can you express its value directly or by referencing another `[vars]` entry?
        - If so, set it in the `[vars]` section.
        - References between entries require Flox CLI 1.14.1 or later.
        - If it requires shell commands or conditional logic, compute and `export` it in the `hook.on-activate` script.
...
For instance, when you install a new package via [`flox install`](/man/flox-install), the CLI is able to make that immediately available to you so there's no need to exit and re-activate.
...
For this tutorial we'll assume that you're an enlightened [fish shell](https://fishshell.com/) user, meaning that we'll edit our `profile.fish` script.
...
-  [Multiple architecture environments](/tutorials/multi-arch-environments)
