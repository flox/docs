...
Let's say that you frequently work on Python projects and use [Poetry](https://python-poetry.org/) as your package manager of choice.
...
Then create a new environment called `python_env` in it with the [`flox init`](/man/flox-init) command.
...
Now install `python312` and `poetry` to this environment with the [`flox install`](/man/flox-install) command:
...
Now you can push this environment to [FloxHub](/concepts/floxhub).
...
You accomplish this with the [`flox pull --copy`](/man/flox-pull) command.
...
Let's create an environment in a `composed_python_project` directory.
...
Run the [`flox edit`](/man/flox-edit) command and make the `include` section of the manifest look like this:
...
If you now run [`flox list`](/man/flox-list), you should see the packages from the `python_env` environment, even though we never ran a `flox install` command on the `composed_python_project` environment!
...
Say that you decide you want to use [hypothesis](https://hypothesis.readthedocs.io/en/latest/) for testing all of your Python projects.
