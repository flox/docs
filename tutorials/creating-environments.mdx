...
You can use Flox to **set up an [environment](/concepts/environments)** for a new
or existing project.
...
[`flox init`](/man/flox-init) command:
...
Once an [environment](/concepts/environments) has been created,
...
environment.
Search for and install packages using [`flox search`](/man/flox-search),
[`flox show`](/man/flox-show), and [`flox install`](/man/flox-install).

Let's assume `example-project` is a nodejs/npm project.
Begin by searching for `nodejs` with [`flox search`](/man/flox-search) in Flox:
...
<Note>
  **Note**
  ...
  Still missing? Reach out to us on our [forum](https://discourse.flox.dev/) for assistance.
</Note>
For more detail about a specific package, such as the available versions,
use [`flox show`](/man/flox-show).
...
[`flox activate`](/man/flox-activate) command to make the packages we installed
available.
...
customized by editing the [environment's declarative manifest](/concepts/environments#manifesttoml)
with [`flox edit`](/man/flox-edit).
This is useful for doing environment initialization,
safely working with secrets,
printing instructions for other developers,
and more.
For a short summary of what the environment is for,
set the manifest's top-level [`description`](/man/manifest.toml#description) instead of printing it from a hook.
...
# Set environment variables. References resolve in dependency order.
[vars]
# message = "Howdy"
# greeting = "$message from Flox"
# pass-in = "${SOME_ENV_VAR}"
...
<Note>
  **Flox CLI version**
  ...
  Flox CLI versions 1.14.1 and later support `[vars]` interpolation.
  Set [`minimum-cli-version`](/man/manifest.toml#minimum-cli-version) to `"1.14.1"`
  in environments that rely on this behavior.
</Note>
...
<Note>
  **Note**
  ...
  Edits made with [`flox edit`](/man/flox-edit) will be validated and built
  immediately.
  Edits made to the [manifest](/concepts/environments#manifesttoml) with external software like
  an IDE will be validated when you run [`flox activate`](/man/flox-activate).
</Note>
Test out the new shell hook by running `exit` and
[`flox activate`](/man/flox-activate) again:
...
We're done!
To exit the last [environment](/concepts/environments) activated,
