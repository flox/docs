...
If that wasn’t cool enough, with Flox each project can have a different CUDA Toolkit installed without
causing version conflicts.
Each project’s dependencies are completely independent of one another.

For a quick overview of Flox, see [Flox in 5 Minutes](/flox-5-minutes).
...
Navigate to `Samples/5_Domain_Specific/Mandelbrot` then run the following commands:

```bash
mkdir build && cd build && cmake .. && make -j8 && ./Mandelbrot
```

This will open a window with a rendering of the [Mandelbrot set](https://en.wikipedia.org/wiki/Mandelbrot_set),
and display some instructions for tweaking the output.
Press the `J` key to switch from the Mandelbrot set to the Julia set, then play around with colors.
Here's an example of what the output can look like after tweaking some of the parameters.
![Julia set rendering generated by the CUDA Mandelbrot sample](/images/julia_set.png)
