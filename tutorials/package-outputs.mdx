...
Packages in the [Flox Catalog](/concepts/packages-and-catalog) contain metadata about their origin, license, version, etc.
...
It's a different story once you start including very large packages like parts of the [CUDA Toolkit](/tutorials/cuda), which add up to several GB.

## Discovering outputs

The main way that you'll discover the outputs of a package is with the [`flox show`](/man/flox-show) command.
...
This is an idiosyncrasy of where the Flox Catalog gets its packages from (Nixpkgs).
...
You can also specify which outputs to install via the [`flox install`](/man/flox-install) command using the following syntax:
