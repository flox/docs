...
This guide walks you through layering a [`default`](/tutorials/default-environment) environment
...
Now let's [`flox install`](/man/flox-install) **tools that will be useful on any
...
Let's inspect the contents of the environment with [`flox list`](/man/flox-list):
...
Use [`flox init`](/man/flox-init) from the `material-ui` directory that we are in.
