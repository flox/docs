...
In order to define a Flox build, we add an entry to the [`[build]`](/man/manifest.toml#build) section of the manifest.
...
See the [builds](/concepts/builds) page for more examples of what you can build with Flox.
...
The `$out` directory adheres to the [Filesystem Hierarchy Standard (FHS)](https://en.wikipedia.org/wiki/Filesystem_Hierarchy_Standard), which is just the formal name for the convention of placing executable files in `/bin`, libraries in `/lib`, etc.
...
Run [`flox edit`](/man/flox-edit) so that you can edit your manifest, and add the following section:
...
It's the moment of truth, let's run [`flox build`](/man/flox-build) to have Flox build our `hello` program for us:
...
See the [organizations](/concepts/organizations) page for more details.

Now that you've built the package you can [publish](/concepts/publishing) it to your private catalog via the [`flox publish`](/man/flox-publish) command.
...
Now that you've published the package, it will show up in [`flox search`](/man/flox-search) and [`flox show`](/man/flox-show), and can be installed via [`flox install`](/man/flox-install).
...
See the [builds](/concepts/manifest-builds#example-configuration-files) concept page for examples of what else you can build and publish with Flox.
